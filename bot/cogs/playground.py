...
class SingleLinePlayground:
    ...
    async def eval(self, ctx: commands.Context, *, code: CodeSection):
        """Evaluates a single line of Rust code. Exactly equal to https://play.integer32.com/"""

        async with ctx.typing():
            payload = json.dumps(
                {
                    "channel": "nightly",
                    "code": 'fn main(){println!("{:?}",{' + code.source + '});}',
                    "crateType": "bin",
                    "mode": "debug",
                    "tests": False,
                }
...
def setup(bot):
    ...
    bot.add_cog(SingleLinePlayground(bot))
